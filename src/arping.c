...
pingmac_send(uint16_t id, uint16_t seq)
{
    ...
    if (-1 == (ipv4 = libnet_build_ipv4(LIBNET_IPV4_H + LIBNET_ICMPV4_ECHO_H + sizeof padding,
                                        0,  /* ToS */
                                        id, /* id */
                                        0,  /* frag */
                                        64, /* ttl */
...
void pingip_recv(const char *unused, struct pcap_pkthdr *h, const char *const packet)
{
    ...
    // in `arping`).
    {
        const uint8_t *p = (u_char *)harp + sizeof(struct libnet_arp_hdr) + ETH_ALEN + IP_ALEN;
        char buf[128];
...
ping_recv(pcap_t *pcap, uint32_t packetwait, pcap_handler func)
{
    ...
    for (; !done;)
    {
        ...
        /* if wait-for-packet time is longer than full period,
                ...
                * it here */
        if ((ts.tv_sec > packetwait / 1000000) || ((ts.tv_sec == packetwait / 1000000) && (ts.tv_nsec / 1000 > packetwait % 1000000)))
        {
            ts.tv_sec = 0;
            ts.tv_nsec = 1;
            done = 1;
...
int arping_main(int argc, char **argv)
{
    ...
    if (((mode == PINGIP) && opt_T) || ((mode == PINGMAC) && (opt_B || dstmac_opt || opt_U)))
    {
    ...
    else if (display == NORMAL)
    {
        ...
        if (numrecvd)
        {
            ...
            printf("rtt min/avg/max/std-dev = "
                   ...
                   1000 * avg,
                   1000 * stats_max_time,
                   1000 * sqrt(stats_total_sq_time / numrecvd - avg * avg));
        }
        printf("\n");
    }
